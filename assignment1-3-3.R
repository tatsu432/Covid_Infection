#データの読み込み
DF <- read.table("covid19_tokyo_daily_last_modified21.csv", 
                 sep = ",", 
                 header = TRUE, 
...
COR7 <- cor(DF[, c(2, 74:79)])
COR7

COR8 <- cor(DF[, c(2, 68:73)])
COR8
...
ggplot(DF) +
  geom_boxplot(aes(x=週の前半後半1, 
                   y=感染者数の増加前日比))
ggplot(DF) +
  geom_boxplot(aes(x=ラグ冬ダミー, 
                   y=感染者数の増加前日比))
ggplot(DF) +
  geom_boxplot(aes(x=ラグ四季ダミー, 
                   y=感染者数の増加前日比))
ggplot(DF) +
  geom_boxplot(aes(x=ラグ四季ダミー, 
                   y=感染者数1日))
...
library(gridExtra)
gridExtra::grid.arrange(infection, 
                        lag_humidity, ncol = 1)
gridExtra::grid.arrange(lag_temperature, 
                        lag_humidity, ncol = 1)
...
lm8 <- lm(感染者数1日 ~ ラグ変数 + 週の前半後半1 + 
                ラグ平均気温 + ラグ平均湿度, data = DF)
summary(lm8)
...
#ワクチン接種者数を加える線形回帰モデルの作成lm12
lm12 <- lm(感染者数1日 ~ ラグ変数 + 週の前半後半1 + 
                 平均気温 + ラグ変数*週の前半後半1 + 
                 ラグ変数*ラグワクチン接種者数, 
               data = DF)
summary(lm12)
...
#緊急事態宣言ダミーと平均湿度を加える線形回帰モデルの作成lm14
lm14 <- lm(感染者数1日 ~ ラグ変数 + 週の前半後半1 + 
                 平均気温 + + 平均湿度 + 
                 ラグ変数*ラグ週の前半後半1 + 
                 ラグ変数*ラグ緊急事態宣言, 
               data = DF)
...
#緊急事態宣言ダミーと平均湿度と冬ダミーを加える線形回帰モデルの作成lm15
lm15 <- lm(感染者数1日 ~ ラグ変数 + 週の前半後半1 + 
                 ラグ平均気温 + ラグ平均湿度 +
                 ラグ緊急事態宣言 + ラグ冬ダミー + 
                 ラグワクチン接種者数, 
               data = DF)
